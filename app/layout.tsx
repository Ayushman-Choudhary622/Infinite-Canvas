              ...
              <div className="flex w-full xl:w-auto gap-2">
                {inputType === "upload" ? (
                  ...
                  <>
                    <input
                      type="text"
                      placeholder={inputType === "embed" ? "Paste <iframe> code here..." : "Type here..."}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 2xl:grid-cols-3 gap-4 md:gap-6 auto-rows-max">
                {activeBoard.elements.map((el) => (
                  <div key={el.id} className="relative group bg-white p-3 md:p-4 rounded-xl shadow-md border border-gray-100">
                    ...
                    {/* Raw HTML Embed Code renderer (Pinterest & Facebook iframes) */}
                    {el.type === "embed" && (
                      <div 
                        className="w-full rounded-lg overflow-hidden flex items-center justify-center bg-gray-100 min-h-[250px] [&>iframe]:w-full [&>iframe]:h-full [&>iframe]:aspect-video"
